...
and tcheck_dec dec tenv =
 ...
 | FunctionDec(f, args, tyopt, exp) ->
     let tenv_2 = extend_tenv_f f (get_fty args tyopt) tenv in
     let tenv_3 = extend_tenv_vs (get_argenv args) tenv_2 in
     let (exp', ty) = tcheck exp tenv_3 in
     let _ = match tyopt with
        ...
        | _ -> ()
     in
     let (f', _) = lookup_fun f tenv_3 in
     let args' = List.map (fun ((v,_), flag, ty) -> (v, flag, ty)) args in
     (Absyn2.FunctionDec(f', args', ty, exp'), tenv_2)
